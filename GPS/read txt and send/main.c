...
INT8 GPS_weeksec_BeiJing(double gpst[2]);
...
int main()
{
    ...
    while (1)
    {
        if(timer_sign == 1) // 定时标志,每次循环等待信号
        {
            ...
            if(fgets(sendtoclient, sizeof(sendtoclient), fp)==0) break;
...
int GPS_Analyse(bdfpd *gps_data, char *read_buff)
{
    ...
    //字符数组名本身就是地址，无需添加&(gps_data.Status)
     sscanf(read_buff,"$BDFPD,%u,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%hd,%hd,%hd,%hd,%c",
     ...
     &((*gps_data).NSV1),&((*gps_data).NSV2),&((*gps_data).PT1),&((*gps_data).PT2), &((*gps_data).checksum));
...
INT8 GPS_weeksec_BeiJing(double gpst[2])
{
    ...
    j0 = j0 + 32083;
    INT32 year0,year,day,month,sec,hour,minute;
